...
public class CardEdge extends Applet
{
	...
	// {
		// new CardEdge().register(bArray, (short) (bOffset + 1), bArray[bOffset]);
	// }
	...
	private CardEdge(byte[] bArray, short bOffset, byte bLength) {
		...
		pins[0] = new OwnerPIN((byte)3, PIN_MAX_SIZE);
		pins[0].update(defaultPIN, (short) 0, (byte)6);
	...
	private void ImportKey(APDU apdu, byte[] buffer) {
		if (buffer[ISO7816.OFFSET_P2] != (byte) 0x00)
			ISOException.throwIt(SW_INCORRECT_P2);              //P2 is 00
		short bytesLeft = Util.makeShort((byte) 0x00, buffer[ISO7816.OFFSET_LC]);
		if (bytesLeft != apdu.setIncomingAndReceive())
			ISOException.throwIt(ISO7816.SW_WRONG_LENGTH);		//Lc correct or not
		byte key_nb = buffer[ISO7816.OFFSET_P1];
		if ((key_nb < 0) || (key_nb >= MAX_NUM_KEYS))
			ISOException.throwIt(SW_INCORRECT_P1);              //why MAXNUMKEYS is 8 
	...
	private void ExternalAuthenticate(APDU apdu, byte[] buffer) {
		if (buffer[ISO7816.OFFSET_P2] != (byte) 0x00)
			ISOException.throwIt(SW_INCORRECT_P2);        		//P2只能为00
		short bytesLeft = Util.makeShort((byte) 0x00, buffer[ISO7816.OFFSET_LC]);
		if (bytesLeft != apdu.setIncomingAndReceive())
			ISOException.throwIt(ISO7816.SW_WRONG_LENGTH);   	//判断Lc是否正确
		byte key_nb = buffer[ISO7816.OFFSET_P1];
		if ((key_nb < 0) || (key_nb >= MAX_NUM_AUTH_KEYS) || (keys[key_nb] == null))
			ISOException.throwIt(SW_INCORRECT_P1);			 	//判断KeyNo是否正确
		if (bytesLeft < 3)
			ISOException.throwIt(SW_INVALID_PARAMETER);		 	//Lc小于3报错
		/* Verify that a GetChallenge has been issued */
		if (!getChallengeDone)
			ISOException.throwIt(SW_OPERATION_NOT_ALLOWED);  	//判是否取随机数
		/*
		 * Clear getChallengeDone flag getChallengeDone = false; /* Retrieve
		 * getChallenge() data position and check it
		 */
		short chall_base = om.getBaseAddress(OUT_OBJECT_CLA, OUT_OBJECT_ID); //??OUT_OBJECT_CLA的定义
		...
		switch (buffer[(short) (ISO7816.OFFSET_CDATA + 2)]) {         //find the input data
		case DL_APDU:
			src_buffer = buffer;
